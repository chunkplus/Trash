...
void getDeviceCount(ipcDevices_t *devices) {
	...
	if (0 == pid) {
		...
		for (i = 1; i < uvaCount; i++) {
			checkCudaErrors(
					cudaDeviceCanAccessPeer(&canAccessPeer_0i, uvaOrdinals[0],
							uvaOrdinals[i]));
			checkCudaErrors(
					cudaDeviceCanAccessPeer(&canAccessPeer_i0, uvaOrdinals[i],
							uvaOrdinals[0]));
...
void runTestMultiKernel(ipcCUDA_t *s_mem, int index) {
	...
	if (index == 0) {
		...
		checkCudaErrors(
				cudaIpcGetMemHandle((cudaIpcMemHandle_t * ) &s_mem[0].memHandle,
						(void * ) d_ptr));
		checkCudaErrors(
		...
		checkCudaErrors(
				cudaMemcpy(h_results, d_ptr + DATA_BUF_SIZE, DATA_BUF_SIZE * (g_processCount - 1) * sizeof(int), cudaMemcpyDeviceToHost));
		...
		checkCudaErrors(
				cudaIpcGetEventHandle(
						(cudaIpcEventHandle_t * ) &s_mem[index].eventHandle,
						event));
		...
		checkCudaErrors(
				cudaIpcOpenMemHandle((void ** ) &d_ptr, s_mem[0].memHandle,
...
int main(int argc, char **argv) {
	...
	if (!IsAppBuiltAs64()) {
		printf(
				"%s is only supported on 64-bit Linux OS and the application must be built as a 64-bit target. Test is being waived.\n",
				argv[0]);
		exit (EXIT_WAIVED);
	}
