...
void runAutoTest(int devID, char **argv, char *ref_file) {
	...
	checkCudaErrors(
			cudaMemcpy(imageData, d_vbo_buffer,
	...
	if (reference_file
			&& !sdkCompareBin2BinFloat("simpleGL.bin", reference_file,
					mesh_width * mesh_height * sizeof(float), MAX_EPSILON_ERROR,
					THRESHOLD, pArgv[0])) {
		g_TotalErrors++;
	}
}
...
void checkResultCuda(int argc, char **argv, const GLuint &vbo) {
	if (!d_vbo_buffer) {
		...
		if (checkCmdLineFlag(argc, (const char **) argv, "regression")) {
			...
			sdkWriteFile<float>("./res/data/regression.dat", data,
